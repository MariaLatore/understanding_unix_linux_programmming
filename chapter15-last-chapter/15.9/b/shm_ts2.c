...
/* create a semset: key 9900, 2 semaphores, and mode rw-rw-rw */
  semset_id = semget (TIME_SEM_KEY, 2, (0666 | IPC_CREAT));
  if (semset_id == -1)
    oops ("semget2", 3);
  ...
  for (n = 0; n < 60; n++)
    {
      ...
      printf ("\tshm_ts2 pid(%d) waiting for lock\n", getpid ());
      wait_and_lock (semset_id);	//lock memory
      printf ("\tshm_ts2 pid(%d) updating memory\n", getpid ());
      strcpy (mem_ptr, ctime (&now));	//write to mem
      sleep (5);
      release_lock (semset_id);	//unlock
      printf ("\tshm_ts2 pid(%d) released lock\n", getpid ());
      sleep (1);		//wati a sec
    }
...
{
  struct sembuf actions[3];	//action set
