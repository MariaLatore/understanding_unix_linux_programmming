/* twebserv.c - a threaded minimal web server (veresion 0.2)
 *      usage: two portnumber
 *   features: supports the GET command only
 *             runs in the current directory
 *             creates a thread to handle each request
 *             supports a special status URL to report interneal state
...
int
get_server_bytes_sent ()
{
  int bytes_sent;
  pthread_mutex_lock (&bytes_lock);
  bytes_sent = server_bytes_sent;
  pthread_mutex_unlock (&bytes_lock);
  return bytes_sent;
}

void
set_server_bytes_sent (int n)
{
  pthread_mutex_lock (&bytes_lock);
  server_bytes_sent = n;
  pthread_mutex_unlock (&bytes_lock);
  ...
  while (1)
    {
      ...
      set_server_requests (get_server_requests () + 1);
...
/*
 * make sure all paths are elow the current directory
 */
void
sanitize (char *str)
{
...
int
http_reply (int fd, FILE ** fpp, int code, char *msg, char *type,
	    char *content)
{
  FILE *fp = fdopen (fd, "w");
  int bytes = 0;
...
do_ls (char *dir, int fd)
{
  ...
  fclose (fp);
  set_server_bytes_sent (get_server_bytes_sent () + bytes);
}
