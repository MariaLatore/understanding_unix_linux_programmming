...
IfStructure *collect_commands ();
void free_if_structure (IfStructure *);
void execute_cmdlist (IfStructure *);
int generate_cmdlist (char ***, IfStructure **);
static char **newstrlist (char **);

void
do_if ()
{
  IfStructure *cmdlist = collect_commands ();
  execute_cmdlist (cmdlist);
...
//get condition cmd region
  while (1)
	...
	continue;
      ...
      if (NULL == splitcmd[0])	//when comline="\n", we'll go into this situation
	{
	  freelist (splitcmd);
	  continue;
	}
...
//get then cmd region
  ...
  while (1)
      ...
      if (0 == ifstack)
	{
      ...
      if (NULL == cmdline)
	continue;
      else if (NULL != (splitcmd = splitline (cmdline)) && NULL == splitcmd[0])	//when comline="\n", we'll go into this situation
	{
	  freelist (splitcmd);
	  continue;
	}
      else if (strcmp (cmdline, "else") == 0)
	{
	  if (ifstack == 1)
	    {
	      new_if_cmds->then_list[i++] = NULL;
...
void
execute_cmdlist (IfStructure * cmdlist)
{
  int i;
  int result;
  ...
  if (result == 0)
	...
	if (strcmp (cmdlist->then_list[i][0], "if") == 0)
	  {
	    if ((nest_cmdlist_len =
		 generate_cmdlist (&cmdlist->then_list[++i],
				   &nest_cmdlist)) > 0)
	      {
		i = i + nest_cmdlist_len - 1;
		execute_cmdlist (nest_cmdlist);
	...
	if (strcmp (cmdlist->else_list[i][0], "if") == 0)
	  {
	    if ((nest_cmdlist_len =
		 generate_cmdlist (&cmdlist->else_list[++i],
				   &nest_cmdlist)) > 0)
	      {
		i = i + nest_cmdlist_len - 1;
		execute_cmdlist (nest_cmdlist);
...
int
generate_cmdlist (char ***arglist, IfStructure ** ret_cmdlist)
{
  int i;
