...
enum states
{ NEUTRAL, IF_SUCCEED, IF_FAILED, SKIPPING_THEN, DOING_THEN, SKIPPING_ELSE,
  DOING_ELSE
};
enum results
{ SUCCESS, FAIL };
 ...
 */
{
  ...
  if (if_state == IF_SUCCEED || if_state == IF_FAILED)
    {
      syn_err ("then expected");
      rv = 0;
    }
  else if (if_state == DOING_THEN || if_state == DOING_ELSE)
    rv = 1;
 ...
 */
{
  ...
  if (strcmp (cmd, "if") == 0)
	...
	{
	  if_state = (if_state == IF_SUCCEED ? DOING_THEN : SKIPPING_THEN);
	  rv = 0;
	}
    }
	...
	{
	  if_state = (if_state == DOING_THEN ? SKIPPING_ELSE : DOING_ELSE);
	  rv = 0;
	}
    }
  else
