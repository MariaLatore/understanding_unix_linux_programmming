...
void
do_post (char *f, int fd)
{
  char *content = "text/plain";
  FILE *fpsock, *fpsockw, *fpfile;
  int c;
  struct stat fileinfo;
  ...
  if (stat (f, &fileinfo) != -1)
    {
      header (fpsock, content);
      fprintf (fpsock, "\r\n");
      fprintf (fpsock, "transmit failed: file exists!\r\n");
      fclose (fpsock);
      exit (1);
    }
  fpfile = fopen (f, "w");
  printf ("the post file is %s\n", f);
  ...
  header (fpsockw, content);
  fprintf (fpsockw, "\r\n");
  fprintf (fpsockw, "transmit success!\r\n");
  fclose (fpsockw);		//should close this first
  fclose (fpsock);		//should colse this at last
  exit (0);
}
