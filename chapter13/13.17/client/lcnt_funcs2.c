...
char *do_transaction ();
int get_clnt_ip (char *buf, int len);
...
int
get_clnt_ip (char *buf, int len)
{

  struct sockaddr_in clntaddr;
  socklen_t sockaddrlen;
  if (0 != getsockname (sd, (struct sockaddr *) &clntaddr, &sockaddrlen))
    {
      narrate ("Cannot get client ip addr", NULL);
      perror ("cannot get udp ip");
      return -1;
    }
  strncpy (buf, inet_ntoa (clntaddr.sin_addr), len);
  return 0;
}
...
validate_ticket ()
{
  char buf[MSGLEN];
  char *response;
  char clntipaddr[MAXIPADDRLEN];

  if (get_clnt_ip (clntipaddr, MAXIPADDRLEN))
    {
      narrate ("FAIL to get clnt addr!", "");
      return -1;
    }
