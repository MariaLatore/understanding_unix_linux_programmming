...
void dostat(char* dir_name,char *filename)
{
	struct stat info;
	char absolute_filename[256]={0};
	snprintf(absolute_filename, sizeof(absolute_filename), "%s/%s", dir_name, filename);
...
void mode_to_letters(int mode, char str[])
{
	...
	if(mode & S_IROTH) str[7]='r';  //3 bits for othre
